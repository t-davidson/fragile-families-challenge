...
bg = pd.read_csv('../../ff_data/background.csv', low_memory=False)#, index_col='challengeID')
bg.cf4fint = ((pd.to_datetime(bg.cf4fint) - pd.to_datetime('1960-01-01')) / np.timedelta64(1, 'D')).astype(int)
...
for c in bg_imputed_cat.columns:
    ...
    new_col = convert_continuous_to_categorical(categories, orig_col, imputed_col)
